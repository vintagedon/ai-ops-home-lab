...
vm_number=7001 #VM ID; it's over 9000!
...
qm create ${vm_number} --name ${vm_name} --ostype l26
function_print_done
...
qm set ${vm_name} --memory ${vm_memory} --cores {$vm_cores} --cpu cputype=${vm_cpu_type}
function_print_done
...
qm set ${vm_name} --serial0 socket --vga serial0
function_print_done
...
qm set ${vm_name} --virtio ${vm_datastore}:0,import-from="$(pwd)/$3",discard=on
function_print_done
...
qm set ${vm_name} --boot order=scsi0 --scsihw virtio-scsi-single
function_print_done

# Set VM agent flag to true, enable 'Automatric TRIM after VM move/migration'
# Enable filesystem freeze/thaw on backup
qm set ${vm_name} --agent enabled=1,fstrim_cloned_disks=1,freeze-fs-on-backup=1

# create a single virtio NIC on the proper bridge
printf "${text_color_yellow}Configuring VM Network...${text_no_color}"
qm set ${vm_name} --net0 virtio,bridge=${vm_network_bridge}
function_print_done

# add a cloud-init device (CD-ROM) as IDE2
qm set $(vm_number) --ide2 ${storage}:cloudinit

# setup network for VM, either DHCP or static
if [ ${vm_dhcp} = 1 ]; then
    printf "${text_color_yellow}Configuring DHCP...${text_no_color}"
    qm set ${vm_name} --ipconfig0 "ip6=auto,ip=dhcp"
else
    printf "${text_color_yellow}Configuring static IP ...${text_no_color/n}"
    qm set ${vm_name} --ipconfig0 ip=${vm_ip},gw=${vm_gateway}
fi
function_print_done
...
if [ ${vm_vlan} > 0  ]; then
    ...
    qm set $(vm_number) --trunks=${vm_vlan}
    function_print_done
fi
function_print_done
...
printf "${text_color_yellow}Creating User and SSH keys...${text_no_color}/n"
 qm set $(vm_number) --ciuser ${vm_username}

# import SSH keys
qm set $(vm_number) --sshkeys ${vm_ssh_key}
function_print_done

# convert the VM to a template
printf "${text_color_yellow}Coverting VM $(vm_number) to a template...${text_no_color}/n"
